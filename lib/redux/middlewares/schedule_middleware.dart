...
Middleware<AppState> _doSchedule() {
  return (Store store, action, NextDispatcher next) async {
    if (action is ScheduleRequest) {
      QuerySnapshot querySnapshot = await Firestore.instance.collection("reservations").orderBy('reservationDateTime').startAt([action.dateTime.add(new Duration(minutes: -30))]).endAt([action.dateTime.add(new Duration(minutes: 30))]).where('doctorId', isEqualTo: action.doctorId).getDocuments();
